# Pantane Research Lab

Technical research documentation covering:

- AI & Machine Learning
- Algorithmic Trading
- FinTech Systems
- Backend Architecture
- Experimental Software Projects

**Author: [Pantane](https://nf-d.netlify.app)**

---

## Structure

- blogs/ → Individual research projects
- assets/ → Shared styling and scripts
- research/ → Future notebooks, models, datasets

---

## License

This Project is under [MIT License](LICENSE)


*Disclaimer: All financial research is for educational purposes only.*

<p align="center">
  <a href="https://nf-d.netlify.app">#denjagidev</a>
</p>

<p align="center">
  <a href="#"><img src="http://readme-typing-svg.herokuapp.com?color=ACAF50&center=true&vCenter=true&multiline=false&lines=Built+Different" alt="pantane"></a>
</p>
